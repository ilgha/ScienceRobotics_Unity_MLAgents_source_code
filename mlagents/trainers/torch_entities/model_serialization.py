...
class ModelSerializer:
    def __init__(self, policy):
        ...
        # --- SwarmACB: use ACTOR observation specs if available (excludes 5-D state) ---
        if hasattr(self.policy, "get_actor_observation_specs"):
            observation_specs = self.policy.get_actor_observation_specs()
            try:
                logger.warning(
                    f"[SwarmACB] Exporter using actor_obs_specs -> "
                    f"{[tuple(s.shape) for s in observation_specs]}"
                )
            except Exception:
                pass
        else:
        ...
        # Expect exactly ONE actor obs for export (your sensor input).
        # Build a single dummy tensor 'obs_0' instead of a list.
        if len(observation_specs) == 0:
            raise RuntimeError("No observation specs available for export.")
        if len(observation_specs) > 1:
            # We still export only the first actor obs; TorchPolicy routes the rest at runtime.
            # If you ever need more, we can extend this, but the request is a single obs_0.
            logger.warning(
                f"[SwarmACB] Exporter: multiple actor obs specs detected; exporting only the first."
            )
        obs0_spec = observation_specs[0]
        dummy_obs0 = torch.zeros(
            batch_dim + list(ModelSerializer._get_onnx_shape(obs0_spec.shape))
        )

        # Action masks (optional input; not an "obs_*" name)
        dummy_masks = torch.ones(
            batch_dim + [sum(self.policy.behavior_spec.action_spec.discrete_branches)]
        )
        # Recurrent memories (optional input; not an "obs_*" name)
        dummy_memories = torch.zeros(
            batch_dim + seq_len_dim + [self.policy.export_memory_size]
        )

        # ---- ONNX tracing inputs: pass a LIST with a SINGLE Tensor as observations ----
        # (This preserves a single obs placeholder "obs_0" but matches the actor's expected input type.)
        self.dummy_input = ([dummy_obs0], dummy_masks, dummy_memories)

        # ---- Input names: exactly one observation placeholder obs_0 ----
        self.input_names = [TensorNames.get_observation_name(0)]  # ['obs_0']
        self.input_names += [
            TensorNames.action_mask_placeholder,
            TensorNames.recurrent_in_placeholder,
        ]

        # Dynamic batch axis for all inputs
        self.dynamic_axes = {name: {0: "batch"} for name in self.input_names}

        # Outputs: keep ML-Agents defaults
        ...
        if self.policy.behavior_spec.action_spec.discrete_size > 0:
            ...
            self.dynamic_axes.update(
                {TensorNames.discrete_action_output: {0: "batch"}}
            )
