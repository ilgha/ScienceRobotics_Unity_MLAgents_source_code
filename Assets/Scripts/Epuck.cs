...
public enum ControllerType { Dandelion, Daisy, Lilly }

[RequireComponent(typeof(Rigidbody))]
[RequireComponent(typeof(BehaviorParameters))]
public class Epuck : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        if (useContinuousActions)
        {
            ...
            if (enableNoise && wheelsNoiseStd > 0f)
            {
                // Left wheel noise
                float leftRange = wheelsNoiseStd * leftWheelVelocity;
                float leftNoise = Random.Range(-leftRange, leftRange);
                leftWheelVelocity += leftNoise;

                // Right wheel noise
                float rightRange = wheelsNoiseStd * rightWheelVelocity;
        ...
        else
        {
            ...
            switch (action)
            {
                ...
                case 1: AutoMoDeControlStepExploration();    break;
                case 2: AutoMoDeControlStepAttraction();     break;
                case 3: AutoMoDeControlStepRepulsion();      break;
                case 4: AutoMoDeControlStepPhototaxis();     break;
                case 5: AutoMoDeControlStepAntiPhototaxis(); break;
            }
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        {
            AutoMoDeStop(); // path blocked => stop
        }
